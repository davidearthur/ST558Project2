...
Cross validation subdivides the training set into $k$ folds, then fits a model using $k - 1$ of those folds, and tests its accuracy predicting on the $k^{th}$ fold.  This is repeated $k - 1$ more times, so that each fold gets a turn as the test set.  The $k$ results (residual sum of squares, etc.) are then averaged.
